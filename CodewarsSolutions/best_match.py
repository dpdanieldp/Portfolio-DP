...
def best_match(goals_1: List[int], goals_2: List[int]) -> int:
    differences = [goals_1[i] - goals_2[i] for i in range(len(goals_1))]
    idx_lowest_dif = [i for i, s in enumerate(differences) if s == min(differences)]
    zam_hi_scores_low_dif = [goals_2[i] for i in idx_lowest_dif]
    idx_to_ret = [i for i, s in enumerate(goals_2) if i in idx_lowest_dif and s == max(zam_hi_scores_low_dif)]
